...
A parameter-free alternative to Hedge/exponential weights using half-normal potential.

Based on "A Parameter-free Hedging Algorithm" by Chaudhuri, Freund, Hsu.
Key features:
1. Uses half-normal potential: φ(x,c) = exp((max{x,0}²)/(2c))
2. Scale c_t found via line search: average potential equals e
3. Actions with negative cumulative regret get zero weight
...
class InformationSetNH:
    """
    Information set for NormalHedge algorithm.
    Tracks cumulative regrets and computes NormalHedge weights.
    """
    def __init__(self, num_actions: int):
        self.num_actions = num_actions
        self.regret_sum = np.zeros(num_actions)  # Cumulative regret R_{I,a}
    ...
    def normalhedge_weight(self, R_plus: np.ndarray, c: float) -> np.ndarray:
        ...
        Args:
            R_plus: Array of nonnegative [R_{I,a}]_+ values
            c: Scale parameter
    ...
    def get_strategy(self) -> np.ndarray:
        ...
        """
        # Compute [R]_+ (positive part of cumulative regrets)
        R_plus = np.maximum(self.regret_sum, 0.0)
    ...
    def add_regret(self, action_idx: int, regret: float):
        ...
        NormalHedge updates cumulative regret directly (no clipping like CFR+).
...
class NormalHedgeTrainer:
    """
    NormalHedge trainer for Kuhn Poker.
    
    Implements parameter-free online learning via NormalHedge algorithm.
    Each information set maintains its own DTOL instance with action regrets.
    
    Key differences from CFR:
    - Uses NormalHedge weighting instead of regret-matching
    - No parameters to tune (scale c_t computed automatically)
    - Actions with negative cumulative regret get exactly zero weight
    """
    def __init__(self, config: GameConfig):
        ...
        """
        self.config = config
        self.info_sets: Dict[str, InformationSetNH] = {}
        self.iteration = 0
    
    def get_info_set(self, key: str, num_actions: int) -> InformationSetNH:
        """Get or create an information set."""
        if key not in self.info_sets:
            self.info_sets[key] = InformationSetNH(num_actions)
    ...
    def train(self, num_iterations: int) -> List[float]:
        ...
        for i in range(num_iterations):
            ...
            value = self.normalhedge_cfr(initial_state, reach_prob_0=1.0, reach_prob_1=1.0)
    ...
    def normalhedge_cfr(self, state: GameState, reach_prob_0: float, reach_prob_1: float) -> float:
        """
        ...
        for i, action in enumerate(legal_actions):
            ...
            if player == 0:
                action_utilities[i] = self.normalhedge_cfr(
                    next_state,
        ...
        # Update regrets using counterfactual regret
